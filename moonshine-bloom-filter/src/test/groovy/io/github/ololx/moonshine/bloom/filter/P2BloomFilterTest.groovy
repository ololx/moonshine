...
class P2BloomFilterTest extends Specification {
    ...
    def "absent - when filter doesn't contain #bytesSupplierValue then return true"() {
        ...
        def bloomFilter = BasicBloomFilter.newInstance(10, [hashFunctionStub1, hashFunctionStub2], 1)
    ...
    def "add - when adding values concurrently then all bits are correctly set"() {
        ...
        def bloomFilter = BasicBloomFilter.newInstance(size, [hashFunctionStub], 1)
