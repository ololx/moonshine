...
class AtomicLongArrayWrapperTest extends Specification {

    def "length() - when get length then return correct length"() {
        given:
        def atomicLongArray = new AtomicLongArrayWrapper([1, 2, 3, 4, 5] as long[])
    ...
    def "getAndSet() - when set then old value returned and new value set"() {
        ...
        when:
        long oldValue = atomicLongArray.getAndSet(0, 7)

        then:
        oldValue == 1L
        atomicLongArray.get(0) == 7
    }
    ...
    def "compareAndSet() - when expect matches then value set and return true"() {
        ...
        when:
        boolean result = atomicLongArray.compareAndSet(0, 1, 8)

        then:
        result
        atomicLongArray.get(0) == 8
    }
    ...
    def "addAndGet() - when add then add value and return new value"() {
        ...
        when:
        long newValue = atomicLongArray.addAndGet(1, 3)
    ...
    def "getAndAdd() - when add then add value and return old value"() {
        ...
        when:
        long oldValue = atomicLongArray.getAndAdd(1, 2)

        then:
        oldValue == 2
        atomicLongArray.get(1) == 4
    }
