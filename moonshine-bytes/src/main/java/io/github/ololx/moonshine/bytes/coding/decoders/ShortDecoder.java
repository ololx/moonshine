...
/**
 * The decoder that converts byte arrays to a {@code Short} value.
 *
 * project moonshine
 ...
 */
public class ShortDecoder implements ValueBytesDecoder<Short> {

    /**
     * Decodes a byte array to a {@code Short} value starting at a given offset
    ...
    public Short decode(byte[] bytes, int offset, ByteIndexOperator endianness) {
        return ValueBytesDecoder.value16BitDecoder().decode(bytes, offset, endianness);
    }
}
